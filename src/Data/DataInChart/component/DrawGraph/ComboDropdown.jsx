...
function ComboDropdown({ type, moreStyle, selectedIndex }) {
  const {
    variables,
    addSelectedMoreYVariableIndexs,
    deleteSelectedMoreYVariableIndexs,
  ...
  const selectDropdownItem = (data) => {
    console.log(data);
    deleteSelectedMoreYVariableIndexs(selectedIndex);
    addSelectedMoreYVariableIndexs(data.variableIndex);
  ...
  if (type === 'y')
    return (
      ...
      >
        ...
        {isOpen &&
          variables.filter(
            (data) => data.isMoreSelected === false && data.type === 'Numeric',
          ).length !== 0 && (
            <div
              style={{
            ...
            >
              {variables
                .filter(
                  (data) =>
                    data.isMoreSelected === false && data.type === 'Numeric',
                )
                .map((data) => (
                  <div
                    key={data.name}
