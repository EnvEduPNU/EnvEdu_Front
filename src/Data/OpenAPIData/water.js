...
function Water() {
  ...
  useEffect(() => {
    customAxios
      .get(`/ocean-quality?year=${selectedYear}&months=${selectedMonth}`)
      .then((jsonData) => {
        ...
        setHeaders(headers);
        const checkedHeaders = Object.keys(jsonData[0]).filter(
          (key) =>
            key !== 'id' &&
            key != 'dataUUID' &&
            key != 'saveDate' &&
  ...
  const handleSaveMyData = async (e) => {
    ...
    console.log(
      '어떻게 저장하는지 좀 보자 : ' + JSON.stringify(selectedItems, null, 2),
    );
  ...
  function handleSelectChange(e) {
    if (e.target.value === '전체') {
      ...
      const selectedStation = data.find(
        (singleData) => singleData.PTNM === e.target.value,
      );
  ...
  const oceanData_ITEMCOD = selectedItems.map(
    (selectedItem) => selectedItem.ITEMCOD,
  );
  const oceanData_ITEMDOC = selectedItems.map(
    (selectedItem) => selectedItem.ITEMDOC,
  );
