        ...
        <div className="login">  
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <fieldset style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <div className="id">
                        <input className="loginInput" placeholder="ID" {...register("username", {required: {value: true, message: "아이디를 입력하세요"}})}/>
                        {errors.username && <span style={{color:'red', fontSize:"13px"}}>{errors.username.message}</span>}
                    </div>
                    <div className="password">
                        <input className="loginInput" placeholder="Password" type="password" {...register("password", {required: {value: true, message: "비밀번호를 입력하세요"}})}/>
