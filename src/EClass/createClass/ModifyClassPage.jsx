...
import { createEclass, getEClassDatas } from './api/eclass';
...
function ModifyClassPage() {
  const { uuid, timestamp } = useParams();
    ...
    const getData = async () => {
      ...
      for (let i = 0; i < eclassData.contents.length; i++) {
        ...
        for (let j = 0; j < step.contents.length; j++) {
          ...
          if (content.type === 'html') {
          ...
          } else if (content.type === 'dataInChartButton') {
            let tableContent;
            let dataContent;
            ...
            if (content.content.dataType === '커스텀 데이터') {
              ...
              if (content.content.dataType === '수질 데이터') {
                ...
                await customAxios
                  ...
                  .catch((err) => console.log(err));
              } else if (content.content.dataType === '대기질 데이터') {
                ...
                await customAxios
                  ...
                  .catch((err) => console.log(err));
              } else if (content.content.dataType === '시도별 대기질 데이터') {
      ...
      // 최종 결과 설정
      setEclassContents(newEclassContents);
      setThumbnailImage(eclassData.thumbImg);
    };
    getData();
  }, [uuid]);
  useEffect(() => {
